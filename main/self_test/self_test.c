...
esp_err_t init_voltage_regulator(GlobalState * GLOBAL_STATE) {
    ...
    // TESTING: Disable TPS546 output completely (0V) for serial testing
    ESP_LOGI(TAG, "Disabling TPS546 for serial testing (normal: %dmV)", nvs_config_get_u16(NVS_CONFIG_ASIC_VOLTAGE));
    ESP_RETURN_ON_ERROR(TPS546_disable(), TAG, "TPS546 disable failed!");
